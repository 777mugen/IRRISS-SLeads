...
async def test_jina_raw_data():
    """测试 Jina API 原始数据版配置"""
    
    print(f"\n{'='*60}")
    print(f"🧪 测试 Jina API 原始数据版配置")
    print(f"{'='*60}\n")
    ...
    async with JinaClient() as client:
        ...
        if image_links:
            print(f"✅ 找到图片链接: {len(image_links)} 个")
        else:
            print(f"❌ 未找到图片链接")
        ...
        if all_links:
            print(f"✅ 找到链接: {len(all_links)} 个（包括图片链接）")
        else:
            print(f"❌ 未找到链接")
        ...
        # 保存到文件
        output_file = Path("tmp/jina_raw_data.txt")
...
if __name__ == "__main__":
    asyncio.run(test_jina_raw_data())
